...
class CloudIpsp {
   ...
   * @returns {*}
   */
  getImportantParams (data, credit = false) {
    data.merchant_id = this.config.merchantId
    data.signature = util.genSignature(data, credit ? this.config.creditKey : this.config.secretKey)
    return data
  }
...
CloudIpsp.prototype.P2pcredit = function (data) {
  ...
  const request = this.getImportantParams(data, true)
  const options = {
    path: 'p2pcredit/',
    body: request
  }
